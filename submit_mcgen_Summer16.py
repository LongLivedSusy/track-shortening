...
if not os.path.exists("CMSSW_7_1_20_patch3"):
    os.system("export SCRAM_ARCH=slc6_amd64_gcc530; cmsrel CMSSW_7_1_20_patch3; cd CMSSW_7_1_20_patch3/src; eval `scramv1 runtime -sh`; scram b -j10; cd -")
if not os.path.exists("CMSSW_8_0_21"):
    os.system("export SCRAM_ARCH=slc6_amd64_gcc530; cmsrel CMSSW_8_0_21; cd CMSSW_8_0_21/src; eval `scramv1 runtime -sh`; scram b -j10; cd -")
...
if step_gensim:
    ...
    GridEngineTools.runParallel(commands, "grid", confirm=True, use_sl6=True)
...
if step_digi:
    ...
    commands = []
    for i, infile in enumerate(glob.glob("/nfs/dust/cms/user/kutznerv/shorttrack/mcgen/Summer16_GENSIM/*root")):
        outfile = "../../" + outdir + "/" + infile.split("/")[-1]
        this_command = example_command.replace("$INFILE", infile).replace("$OUTFILE", outfile).replace("$NEV", "1000").replace("$NUM", str(i))
        commands.append(this_command)
...
if step_reco:
    ...
    commands = []
    for i, infile in enumerate(glob.glob("/nfs/dust/cms/user/kutznerv/shorttrack/mcgen/Summer16_DIGI/*root")):
        outfile = "../../" + outdir + "/" + infile.split("/")[-1]
        this_command = example_command.replace("$INFILE", infile).replace("$OUTFILE", outfile).replace("$NEV", "1000").replace("$NUM", str(i))
        commands.append(this_command)
...
if step_rereco:
    ...
    commands = []
    for i, infile in enumerate(glob.glob("/nfs/dust/cms/user/kutznerv/shorttrack/mcgen/Summer16_RECO/*root")):
        outfile = "../../" + outdir + "/" + infile.split("/")[-1]
        this_command = example_command.replace("$INFILE", infile).replace("$OUTFILE", outfile).replace("$NEV", "1000").replace("$NUM", str(i))
        commands.append(this_command)
