...
def allperiods(histofolder, plotfolder, suffix, use_uncertainty_from_teff = False, lumi_weighting = 1, use_exact_layer_matching = False):
    ...
    for category in [
        ...
        for period in periods:
            ...
            hists[mcperiod]["h_mcfinaleff"].Divide(hists[mcperiod]["h_tracks_rereco" + category])
        ...
        for year in ["2016", "2017", "2018"]:  
            ...
            quickdraw({"Data": hists[period]["h_finaleff"]}, hists[mcperiod]["h_mcfinaleff"], "plots/ratios-before-teff" + category + year + ".pdf", legendheader = category.replace("_", "") + " tracks", mclabel = "MC", title = "# tagged / # reco;layers;ratio", ymin = 0, ymax = 1)
            ...
            quickdraw({"Data": hists[period]["h_finaleff"]}, hists[mcperiod]["h_mcfinaleff"], "plots/ratios-after-teff" + category + year + ".pdf", mclabel = "MC", title = "# tagged / # reco, using TEfficiency;layers;ratio", ymin = 0, ymax = 3)
            ...
            quickdraw({"SF": h_scalefactor_noteff}, False, "plots/sf-noteff" + category + year + ".pdf", title = ";layers;SF", ymin = 0, ymax = 2)
            quickdraw({"SF, with TEfficiency": hists[period]["h_scalefactor"]}, False, "plots/sf-teff" + category + year + ".pdf", title = ";layers;SF", ymin = 0, ymax = 2)
            ...
            fitresults["fit_sf_lumisam"][year + category] = g1.GetParameter(0)
            #fitresults["fit_uncert_lumisam"][year + category] = g1.GetParError(0)
            if "short" in category:
                fitresults["fit_uncert_lumisam"][year + category] = hists[period]["h_scalefactor"].GetBinError(4)
            elif "long" in category:
                fitresults["fit_uncert_lumisam"][year + category] = hists[period]["h_scalefactor"].GetBinError(6)
            else:
                fitresults["fit_uncert_lumisam"][year + category] = hists[period]["h_scalefactor"].GetBinError(6)
            ...
            legend.Draw()
            canvas.SaveAs("plots/fit_sfsam_%s%s.pdf" % (year, category))
            #raw_input("@@@@@ g1.GetParameter(0) " + str( g1.GetParameter(0)) + " plots/fit_sfsam_%s%s.pdf" % (year, category))
...
if __name__ == "__main__":
    ...
    plotfolder = "plots-wrong%s" % options.suffix
